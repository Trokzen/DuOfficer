            ...
            if conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
                        ...
                        FROM app_schema.action_reference_files arf
                        JOIN app_schema.organization_reference_files orf ON arf.file_id = orf.id
                        JOIN app_schema.organizations o ON arf.organization_id = o.id
                        WHERE arf.action_id = %s 
                        ORDER BY o.name, orf.file_type, orf.file_path;
                    """, (action_id,))
                    rows = cursor.fetchall()
                    files = [dict(row) for row in rows]
                    logger.info(f"PostgreSQLDatabaseManager: Получено {len(files)} файлов для действия ID {action_id}.")
                    return files
        except Exception as e:
            logger.error(f"PostgreSQLDatabaseManager: Ошибка при получении файлов для действия ID {action_id}: {e}")
            return []
        except Exception as e:
            logger.exception(f"PostgreSQLDatabaseManager: Неизвестная ошибка при получении файлов для действия ID {action_id}: {e}")
            return []
        return []
    ...
    def add_action_reference_file(self, action_id: int, organization_id: int, file_id: int) -> bool:
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        """INSERT INTO app_schema.action_reference_files (action_id, organization_id, file_id) 
                           VALUES (%s, %s, %s) 
                           ON CONFLICT (action_id, file_id) DO NOTHING;""",
                        (action_id, organization_id, file_id)
                    )
                    conn.commit()
                    affected_rows = cursor.rowcount
                    logger.info(f"PostgreSQLDatabaseManager: Привязан файл ID {file_id} к действию ID {action_id}.")
                    return affected_rows > 0
        except Exception as e:
            logger.error(f"PostgreSQLDatabaseManager: Ошибка при привязке файла к действию ID {action_id}: {e}")
            if conn:
                conn.rollback()
            return False
        except Exception as e:
            logger.exception(f"PostgreSQLDatabaseManager: Неизвестная ошибка при привязке файла к действию ID {action_id}: {e}")
            if conn:
                conn.rollback()
            return False
        return False
    ...
    def remove_action_reference_file(self, action_id: int, file_id: int) -> bool:
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "DELETE FROM app_schema.action_reference_files WHERE action_id = %s AND file_id = %s;",
                        (action_id, file_id)
                    )
                    conn.commit()
                    affected_rows = cursor.rowcount
                    logger.info(f"PostgreSQLDatabaseManager: Отвязан файл ID {file_id} от действия ID {action_id}.")
                    return affected_rows > 0
        except Exception as e:
            logger.error(f"PostgreSQLDatabaseManager: Ошибка при отвязке файла от действия ID {action_id}: {e}")
            if conn:
                conn.rollback()
            return False
        except Exception as e:
            logger.exception(f"PostgreSQLDatabaseManager: Неизвестная ошибка при отвязке файла от действия ID {action_id}: {e}")
            if conn:
                conn.rollback()
            return False
        return False
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute("SELECT id, name FROM app_schema.organizations ORDER BY name;")
                    ...
                    for org in orgs:
                        ...
                        cursor.execute("""
                            ...
                            FROM app_schema.organization_reference_files 
                        ...
                        cursor.execute("""
                            SELECT file_id FROM app_schema.action_reference_files 
                            WHERE action_id = %s AND organization_id = %s;
                    ...
                    logger.info(f"PostgreSQLDatabaseManager: Получено {len(result)} организаций с файлами для действия ID {action_id}.")
                    return result
        except Exception as e:
            logger.error(f"PostgreSQLDatabaseManager: Ошибка при получении организаций для действия ID {action_id}: {e}")
            return []
        except Exception as e:
            logger.exception(f"PostgreSQLDatabaseManager: Неизвестная ошибка при получении организаций для действия ID {action_id}: {e}")
            return []
        return []
